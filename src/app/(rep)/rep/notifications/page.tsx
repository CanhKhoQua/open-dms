...
import { markAllReadRep } from "./actions";
...
export default async function RepNotificationsPage() {
  const rep = await getCurrentRep();
  const items = await getNotifications(rep.id);
  ...
  return (
    <main className="px-5 pt-6 pb-6">
      ...
      <section className="mt-4 flex flex-col gap-2">
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2">
                <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-semibold ${kind.cls}`}>
                  {kind.label}
                </span>
        ...
        {items.length === 0 && (
          <p className="text-[13px] text-gray-400 py-8 text-center">No notifications.</p>
        )}
      </section>
    </main>
  );

...
type ManagerHref = "/manager" | "/manager/debt" | "/manager/team" | "/manager/visits";
...
export function ManagerShell({ managerName, children }: ManagerShellProps) {
  const pathname = usePathname();
  const activeHref = RAIL_ITEMS.find((i) => isActive(i.href, pathname))?.href ?? "";
  const meta = ROUTE_META[activeHref] ?? { title: "Open DMS", subtitle: "" };
    ...
    <div className="flex h-screen overflow-hidden bg-gray-50">
      ...
      <aside className="w-[216px] flex flex-col bg-white shrink-0 border-r border-gray-100">
        ...
        <div className="flex items-center gap-2.5 px-4 pt-5 pb-[22px]">
          ...
          <div>
            <div className="text-[16px] font-bold tracking-tight text-gray-900 leading-none">
              Open DMS
            </div>
            <div className="text-[10px] font-semibold tracking-[0.08em] uppercase mt-[3px] text-tnm-teal-500">
              Manager
            </div>
          </div>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div>{item.label}</div>
                  <div className={`text-[10px] mt-0.5 ${active ? "text-blue-600 opacity-70" : "text-gray-400"}`}>
                    {item.sub}
                  </div>
                </div>
              </Link>
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        ...
        <header className="h-[60px] flex items-center gap-4 px-6 bg-white shrink-0 border-b border-gray-100">
          <div className="min-w-0">
            <div className="text-[17px] font-semibold tracking-tight text-gray-900 leading-tight">
              {meta.title}
            </div>
            {meta.subtitle && (
